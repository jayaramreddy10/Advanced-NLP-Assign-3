...
class AGNews_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #get label/target
        fwd_labels = []
        fwd_label_sentence = prefix_sentence[1:] + ['<pad>']
        for word in fwd_label_sentence:
            if word in self.word_emb_model.stoi:
                # If the word is in the GloVe vocabulary, get its label
                label = self.vocab[word]
            else:
                # If the word is not in the GloVe vocabulary, use the label of unk
                label = self.vocab[unk_token]
            fwd_labels.append(label)
        fwd_labels = np.array(fwd_labels)
        # print(labels)     
        ...
        #get label/target
        rev_labels = []
        rev_label_sentence = rev_prefix_sentence[1:] + ['<pad>']
        for word in rev_label_sentence:
            if word in self.word_emb_model.stoi:
                # If the word is in the GloVe vocabulary, get its label
                label = self.vocab[word]
            else:
        ...
        # rev_labels = [self.vocab[word] for word in rev_label_sentence]  #vocab is dict (maps words to labels)
        rev_labels = np.array(rev_labels)
        
        return ((fwd_prefix_emb, fwd_labels), (rev_prefix_emb, rev_labels))
        # return sentence
...
class ELMo(nn.Module):
    ...
    def forward(self, x_fwd, x_rev):
        ...
        #map to vocab size
        out_fwd2 = self.W(out_fwd2)   #(seq_len, batch_size, vocab_size) 
        out_rev2 = self.W(out_rev2)   #(seq_len, batch_size, vocab_size) 
...
def train_network(args, train_dataset, val_dataset, vocabulary, word_emd_model):
    ...
    for i in range(args.epochs):
        ...
        trainer.train_ELMo(device, i, n_train_steps=args.number_of_steps_per_epoch)
...
original_list = data_train['Description'].tolist()
random.shuffle(original_list)
# Calculate the split index (e.g., 50%)
split_index = len(original_list) // 6
# Split the list into two parts
val = original_list[:split_index]   #20k
train = original_list[split_index:]  #100k
...
corpus = " ".join(original_list)   #combine all sentences into single string
...
# sorted_vocab = ["sos","eos","unk", "<pad>"]
# sorted_vocab.extend([pair[0] for pair in vocab.most_common()])
# text = "\n".join(sorted_vocab)
# fp = open("AG_data/vocab.txt","w")
# fp.write(text)
# fp.close()

# train_vocab_len = len(vocab)
# vocab['unk'] = train_vocab_len # 1 for safety
val_max_len = 0
val_sentences = []
for i, text in  enumerate(val):
    val[i] = val[i].lower()
    ...
    words = val[i].split(" ")
    val_max_len = max(val_max_len, len(words))
...
train_dataset = AGNews_Dataset(train_sentences, window_size, vocab, word_emd_model)
val_dataset = AGNews_Dataset(val_sentences, window_size, vocab, word_emd_model)
