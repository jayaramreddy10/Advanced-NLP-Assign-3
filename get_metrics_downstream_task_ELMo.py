...
# ID = 'ELMo_training_' + str(number)
# run = wandb.init(project='ELMo_training', name = NAME, id = ID)
...
def get_predicted_labels(model, dataloader):
    ...
    with torch.no_grad():
        for batch_fwd,  batch_rev, targets in dataloader:
            # time_s = time.time()
            ...
            batch_fwd = einops.rearrange(batch_fwd, 'b w e -> w b e')   #ex: 52-w, 256-b, 300-e
            batch_rev = einops.rearrange(batch_rev, 'b w e -> w b e')   #ex: 52-w, 256-b, 300-e

            # NNLM: batch[1].size: (B, )
            # LSTM: batch[1].size: (B, window_size/seq_len)
            batch_fwd = to_device(batch_fwd)
            batch_rev = to_device(batch_rev)
            
            logits = model(batch_fwd, batch_rev)   #logits in this case (B, num_classes) 
            # Apply softmax to convert logits to probabilities
            probs = F.softmax(logits, dim=1)
            
            # Get the class with the highest probability as the predicted label
            batch_predicted_labels = torch.argmax(probs, dim=1).tolist()
...
def report_classification_metrics(args, train_dataloader, val_dataloader, test_dataloader, vocabulary, word_emd_model):
    #load model architecture 
    vocab_size = len(vocabulary)  # Size of your vocabulary

    ########################################################################################################################################################
    #ELMo model is needed to load embeddings
    ...
    ########################################################################################################################################################
    #load classification downstream model

    ckpt_path = "/home/jayaram/research/NLP_course_papers_and_content/Assignments/Assign_2"
    checkpoint_path = ckpt_path + "/state_0.pt"
    ...
    train_predicted_labels, train_target_labels = get_predicted_labels(classification_model, train_dataloader)
    val_predicted_labels, val_target_labels = get_predicted_labels(classification_model, val_dataloader)
    test_predicted_labels, test_target_labels = get_predicted_labels(classification_model, test_dataloader)

    # get all the scores
    # Calculate accuracy
    train_accuracy = accuracy_score(train_target_labels, train_predicted_labels)
    # Calculate micro F1 score
    train_micro_f1 = f1_score(train_target_labels, train_predicted_labels, average='micro')
    # Print the results
    print("Train Accuracy:", train_accuracy)
    print("Train Micro F1 Score:", train_micro_f1)

    val_accuracy = accuracy_score(np.array(val_target_labels), np.array(val_predicted_labels))
    # Calculate micro F1 score
    val_micro_f1 = f1_score(np.array(val_target_labels), np.array(val_predicted_labels), average='micro')
    # Print the results
    print("val Accuracy:", val_accuracy)
    print("val Micro F1 Score:", val_micro_f1)

    test_accuracy = accuracy_score(test_target_labels, test_predicted_labels)
    # Calculate micro F1 score
    test_micro_f1 = f1_score(test_target_labels, test_predicted_labels, average='micro')
    # Print the results
...
####################################################################################################################################
csv_file = 'AG_data/test.csv'
data_test = pd.read_csv(csv_file, header=0, names=['Class Index', 'Description'])   #120k rows
# data_train['Description'] = data_train['Description'] + " ."  (every sentence already has . at the end)
test_sentence_list = data_test['Description'].tolist()
test_labels_list = data_test['Class Index'].tolist()
test_labels_list = [x - 1 for x in test_labels_list]# random.shuffle(sentence_list)

test_sentences = []
test_labels_filtered = []
for i, text in  enumerate(test_sentence_list):
    test_sentence_list[i] = test_sentence_list[i].lower()
    test_sentence_list[i] = test_sentence_list[i].strip('\n')
    test_sentence_list[i] = re.sub(r"(@\[A-Za-z0-9]+)|([^0-9A-Za-z \n])|(\w+:\/\/\S+)|^rt|www.+?", " ", test_sentence_list[i])

    words = test_sentence_list[i].split(" ")

    if(len(words) <= max_seq_len):
        test_data_processed = process_data(words, vocab)    # use unk token for all unseen words in test corpus
        test_sentences.append(test_sentence_list[i])
        test_labels_filtered.append(test_labels_list[i])
   
print('total no of test sentences after filtering: {}'.format(len(test_sentences)))
test_str = " ".join(test_sentences)   #combine all test sentences into single string
test_data_words = split_into_words(test_str)
print("Number of tokens in test data = ",len(test_data_words))
...
test_dataset = AGNews_Dataset(test_sentences, test_labels_filtered, window_size, vocab, word_emd_model)
...
test_dataloader = torch.utils.data.DataLoader(
    test_dataset, batch_size=batch_size, num_workers=1, shuffle=True, pin_memory=True
)
# train_dataloader = torch.utils.data.DataLoader(
...
report_classification_metrics(args, train_dataloader, val_dataloader, test_dataloader, vocab, word_emd_model)
