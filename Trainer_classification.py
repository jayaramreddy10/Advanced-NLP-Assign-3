...
class Trainer_classification_jayaram(object):  
    def __init__(
        self,
        model,
        train_dataset, 
    ...
    def train_classification_ELMo(self, device, epoch_no, n_train_steps):
        ...
        for step in range(n_train_steps):
            ...
            for i in range(self.gradient_accumulate_every):
                batch_fwd, batch_rev, targets = next(self.train_dataloader)   #add collate fn here (not necessary, handled in __get_item itself)
                ...
                logits = self.model(batch_fwd, batch_rev)   #logits in this case (B, num_classes) 
                # outputs = einops.rearrange(outputs, 'w b e -> b w e')   #(B, seq_len, num_classes)
                ...
                loss.backward()   #will train 2 layers of fwd LSTM layers

            #scale back loss
            loss = loss * self.gradient_accumulate_every
            wandb.log({'train loss classification LSTM': loss, 'epoch': epoch_no, 'step no': step}) #, 'batch': t})
        ...
        with torch.no_grad():
            for val_batch_fwd,  val_batch_rev, val_targets in self.val_dataloader:
                # time_s = time.time()
                ...
                # NNLM: batch[0].size: (B, window_size*embed_size)
                # LSTM: batch[0].size: (B, window_size, embed_size), we need to reshape this to ( window_size, B, embed_size) to pass it to LSTM
                val_batch_fwd = einops.rearrange(val_batch_fwd, 'b w e -> w b e')   #ex: 52-w, 256-b, 300-e
                val_batch_rev = einops.rearrange(val_batch_rev, 'b w e -> w b e')   #ex: 52-w, 256-b, 300-e

                # NNLM: batch[1].size: (B, )
                # LSTM: batch[1].size: (B, window_size/seq_len)
                val_batch_fwd = to_device(val_batch_fwd)
                val_batch_rev = to_device(val_batch_rev)
                
                val_logits = self.model(val_batch_fwd, val_batch_rev)   #logits in this case (B, num_classes) 
                # outputs = einops.rearrange(outputs, 'w b e -> b w e')   #(B, seq_len, num_classes)
                # outputs = outputs.view(-1, outputs.size(-1))      

                # compute fwd and rev losses and backpropogate to train fwd_LSTM and rev_LSTM seperately.
                val_loss = self.criterion(val_logits, val_targets)
        ...
        print(f'Validation Loss classification LSTM: {average_validation_loss:.4f}, Epoch: {epoch_no}')
        wandb.log({'val loss classification LSTM': average_validation_loss, 'epoch': epoch_no}) #, 'batch': t})
